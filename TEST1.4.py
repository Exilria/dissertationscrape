...
def main():
    ...
    c_names = pd.read_csv("vtuber_channels_test4.csv", names=["vtuber_name", "affiliation", "channel_id","gender","language"])
    ...
    for index, row in c_names.iterrows():
        ...
        for vid in all_vids_details:
            ...
            continue
        print(data_we_want)
        df.to_csv('./data_csvs4/data_'+str(index)+'.csv', index=False)
